...
export async function OverviewDashboard({ stats }: OverviewDashboardProps) {
  // If stats are passed, use them (for Supabase-based pages)
  if (stats) {
  ...
  // Try to fetch from Supabase if no stats provided
  try {
    ...
    const roadsNeeding =
      iriData?.filter((r) => r.iri > 150).length || defaults.roadsNeeding;
