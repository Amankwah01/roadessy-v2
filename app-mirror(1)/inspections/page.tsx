...
async function fetchInspectionData() {
  ...
  try {
    const { data, error } = await supabase
      .from("iri_sample")
      .select(
        "id, road_name, iri, iri_inst, iri_smartphone, speed_inst, speed_smartphone, vert_displacement, travel_distance, road_type, region, road_condition, pci_score",
      )
      .order("id");
...
async function fetchStats() {
  ...
  try {
    const [
      { data: totalData, count: totalCount },
      { data: completedData, count: completedCount },
    ...
    const avgIri =
      iriData && iriData.length > 0
        ? Math.round(
            iriData.reduce((sum, row) => sum + (row.iri || 0), 0) /
              iriData.length,
          )
        : 0;
